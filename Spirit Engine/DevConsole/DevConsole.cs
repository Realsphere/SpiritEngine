...
namespace Realsphere.Spirit.DeveloperConsole
{
    ...
    public class DevCommand
    {
        ...
        public DevCommand(string name, EventHandler<object[]> onexec = null)
        {
            this.Name = name;
            this.OnExecute = onexec;
        }
    }
    ...
    public static class DevConsole
    {
        ...
        internal static List<string> cmdTxt = new();

        public static EventHandler OnShow;
        public static EventHandler OnHide;

        internal static void execute(string cmdline)
        {
            foreach (DevCommand cmd in commands)
            {
                if (cmd.Name == (cmdline.Contains(" ") ? cmdline.Split(" ").First() : cmdline))
                    if(cmd.OnExecute != null) cmd.OnExecute(cmd, cmdline.Split(" ").Except(new[] {cmdline.Split(" ").First()}).ToArray());
            }
        }
        ...
        {
            show = !show;
            Game.CameraLookActive = !show;
            if(show)
                if (OnShow != null) OnShow(null, new());
            else
                if (OnHide != null) OnHide(null, new());
        }
