...
namespace Realsphere.Spirit.Internal.Common
{
    internal static class TextureLoader
    {
        ...
        public static Texture2D CreateTexture2DFromStream(Device device, MemoryStream source, int width, int height)
        {
            Texture2DDescription desc;
            desc.Width = width;
            desc.Height = height;
            ...
            var s = new DataStream(height * width * 4, true, true);
            s.Write(source.ToArray(), 0, source.ToArray().Length);

            var rect = new DataRectangle(s.DataPointer, width * 4);
        ...
        public static SharpDX.Direct3D11.ShaderResourceView ShaderResourceViewFromFile(Device device, string filename)
        {
            ...
            using (var tex = CreateTexture2DFromFile(device, filename))
            {
                ...
                srv = new SharpDX.Direct3D11.ShaderResourceView(device, tex);
        ...
        public static SharpDX.Direct3D11.ShaderResourceView ShaderResourceViewFromStream(Device device, MemoryStream str, float w, float h)
        {
            SharpDX.Direct3D11.ShaderResourceView srv;

            using (var tex = CreateTexture2DFromStream(device, str, (int)w, (int)h))
            {
                srv = new SharpDX.Direct3D11.ShaderResourceView(device, tex);
            }
