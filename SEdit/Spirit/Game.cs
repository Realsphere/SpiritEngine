...
namespace Realsphere.Spirit
{
    ...
    public static class Game
    {
        ...
        public static void StartGame(SpiritStartInfo ssi)
        {
            ...
            try { File.WriteAllBytes("d3dcompiler_46.dll", SEdit.Properties.Resources.d3dcompiler_46); } catch(IOException) { }
            try { File.WriteAllBytes("d3dcsx_46.dll", SEdit.Properties.Resources.d3dcsx_46); } catch (IOException) { }
            try { File.WriteAllBytes("libbulletc.dll", SEdit.Properties.Resources.libbulletc); } catch (IOException) { }
            try { File.WriteAllBytes("soft_oal.dll", SEdit.Properties.Resources.soft_oal); } catch (IOException) { }
            try { File.WriteAllBytes("libsndfile-1.dll", SEdit.Properties.Resources.libsndfile_1); } catch (IOException) { }
            try { File.WriteAllBytes("ogg.dll", SEdit.Properties.Resources.ogg); } catch (IOException) { }
            try { File.WriteAllBytes("vorbis.dll", SEdit.Properties.Resources.vorbis); } catch (IOException) { }
            try { File.WriteAllBytes("vorbisenc.dll", SEdit.Properties.Resources.vorbisenc); } catch (IOException) { }
            try { File.WriteAllBytes("flac.dll", SEdit.Properties.Resources.flac); } catch (IOException) { }
            ...
            try
            {
                ...
                var t1 = new Thread(() =>
                {
                    ...
                    while (!app.Window.IsDisposed)
                        if (PhysicsEngine.running && !PhysicsEngine.pause) PhysicsEngine.step();
                });
        ...
        static void InputThread()
        {
            ...
            while (!app.Window.IsDisposed)
            {
                if (DeveloperConsole.DevConsole.show)
                {
                    Cursor.Show();
                    continue;
                }
